...
public class Explorer extends MyUnit {
    ...
    boolean isSafeFromBase(Location loc) {
        ...
        return loc.distanceSquared(comm.baseLocEnemy) > UnitType.BASE.getStat(UnitStat.ATTACK_RANGE) || !uc.isObstructed(loc, comm.baseLocEnemy, uc.getOpponent()); // what if obstructed? i guess we don't care
    }
    ...
    boolean rushEnemy() {
        ...
        for (UnitInfo enemy : enemies) {
            if (enemy.getType() != UnitType.RANGER && enemy.getType() != UnitType.EXPLORER || enemy.getLevel() == 2) return false;
            if (uc.isObstructed(nearestRanger, myLoc)) continue;
    ...
    void tryMove() {
        if (uc.getInfo().getHealth() < REINFORCE_HEALTH) {
            healing = true;
        }
        if (healing && uc.getInfo().getHealth() == uc.getInfo().getStat(UnitStat.MAX_HEALTH)) healing = false;
        Location loc = null;
        if (comm.task == 1 || healing) loc = comm.baseLoc;
        if (comm.task == 2) {
